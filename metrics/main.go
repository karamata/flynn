...
func run(ctx context.Context, jobs []*job) error {
	if err := writeGlobalConfig(jobs); err != nil {
		return err
	}

	if err := configureJobs(ctx, jobs); err != nil {
		return err
	}
...
func writeGlobalConfig(jobs []*job) error {
	...
	if err := globalConfig.Execute(io.MultiWriter(configFile, os.Stdout), &globalConfigData{jobs}); err != nil {
		...
		for {
			...
			case event, ok := <-events:
				...
				case discoverd.EventKindUp:
					instances[event.Instance.Addr] = struct{}{}
...
func writeJobConfig(job *job, instances map[string]struct{}) error {
	config := &jobConfig{
		Targets: make([]string, 0, len(instances)),
		Labels:  map[string]string{},
	}
	for inst := range instances {
		config.Targets = append(config.Targets, inst)
	}
	data, err := json.Marshal([]*jobConfig{config})
	if err != nil {
		return err
	}
