...
usage: flynn cluster
       flynn cluster add [-f] [-d] [--git-url <giturl>] [--no-git] [--tar-url <tarurl>] [--docker-push-url <url>] [--docker] [-p <tlspin>] <cluster-name> <domain> <key>
...
func runCluster(args *docopt.Args) error {
	...
	listRec(w, "NAME", "CONTROLLER URL", "GIT URL", "TAR URL")
	for _, s := range config.Clusters {
		...
		data := []interface{}{s.Name, s.ControllerURL, gitURL, tarURL}
...
func runClusterAdd(args *docopt.Args) error {
	...
	if s.DockerPushURL != "" {
		fmt.Fprintln(os.Stderr, "DEPRECATED: Pushing via a Docker registry has been deprecated in favour of pushing via the tar service, set --tar-url instead\n")
	...
	for {
		...
		case event, ok := <-events:
			...
			if e.DomainMigration.FinishedAt != nil {
				...
				cluster.TarURL = fmt.Sprintf("https://tar.%s", dm.Domain)
